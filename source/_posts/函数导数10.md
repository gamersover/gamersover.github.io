...
证：不妨设$\frac{\partial F(x_0,y_0)}{\partial y} > 0$，由条件$(a)$可知，存在一个包含$(x_0,y_0)$的开矩形$I^\prime \times J$，满足$I^\prime \times \bar J \subset D$，且在$I^\prime \times \bar J$上有$\frac{\partial F}{\partial y} > 0$。从而对任意给定的$x \in I^\prime$，$F(x,y)$在闭区间$\bar J$上是严格递增的连续函数。设$J = (c,d)$，由条件$(b)$可知必有
$$
...
证：由[定理1](todo)的证明，可知（1）（2）的证明方式一模一样；而在证明（3）（4）时，只需令$\boldsymbol{h} = (0, \cdots, h_i, \cdots, 0)^T$，固定$x_i$，其他证明过程一样，即可证得（3）和（4）。
...
证：对方程组的个数$m$进行归纳。当$m=1$时，此时该定理即[定理2](todo)。先设方程组个数为$m-1$时该定理成立，再证明在$m$时依然成立即可。<br/>
...
$$
由[定理2](todo)可知，存在$(\boldsymbol{x}_0, \boldsymbol{u}_0, t_0)$的一个领域$(G_n \times G_{m-1}) \times J \subset D^\prime$，使得：<br/>
