...
证：由[定理1](https://gamersover.github.io/2021/04/13/函数导数10/#定理1：隐函数定理)的证明，可知（1）（2）的证明方式一模一样；而在证明（3）（4）时，只需令$\boldsymbol{h} = (0, \cdots, h_i, \cdots, 0)^T$，固定$x_i$，其他证明过程一样，即可证得（3）和（4）。
...
证：对方程组的个数$m$进行归纳。当$m=1$时，即为该定理[定理2](https://gamersover.github.io/2021/04/13/函数导数10/#定理2)。先设方程组个数为$m-1$时该定理成立，再证明在$m$时依然成立即可。
...
$$
由[定理2](https://gamersover.github.io/2021/04/13/函数导数10/#定理2)可知，存在$(\boldsymbol{x}_0, \boldsymbol{u}_0, t_0)$的一个邻域$(G_n \times G_{m-1}) \times J \subset D^\prime$，使得：
