...
证：任取$x \in \bigcup \limits_{n=1}^\infty D_{1/n}$，知$\omega_f(x) > 0$，从而由[定理4](#定理4)可知$x$是$f$的不连续点，从而
$$
...
证：必要性。只需要证明对任意的$\delta > 0$，$D_{\delta}$是零测集，从而$D_1,D_{1/2},\cdots$都是零测集，再由[定理5](#定理5)可知$D(f)$是零测集。由于$f$可积，从而由[函数积分三的定理5](https://gamersover.github.io/2021/05/14/函数积分3/#定理5)可知对任意给定的$\varepsilon > 0$，存在$[a,b]$上的一个分割
$$
...
$$
由[定理6](#定理6)可知，对上述的$\varepsilon > 0$，存在$\delta > 0$， 使得当$x \in K, y \in [a, b]$且$|x - y| < \delta$时，有
$$
...
$$
其中$\sum_1$表示对满足$K \cap (x_{i-1}, x_i) \ne \varnothing$的$i$的求和，$\sum_2$表示对满足$K \cap (x_{i-1}, x_i) = \varnothing$的$i$的求和。对$\sum_1$中的项，任取$y_i \in K \cap (x_{i-1}, x_i)$，由[定理6](#定理6)得
$$
    \begin{aligned}
