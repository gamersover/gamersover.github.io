...
证：$（2）\Rightarrow（3）$显然；$（3）\Rightarrow （4）$由[定理6](todo)可知；$（4）\Rightarrow （1）$由[定理7](todo)可知；下面证$（1）\Rightarrow （2）$。假设$f$在$I$上的积分值为$A$，由可积的定义知，对任意的$\varepsilon > 0$，存在$\delta > 0$，使得对于分割$\pi = \{ I_1, I_2, \cdots, I_k \}$满足$\Vert \pi \Vert < \delta$，则对任意的值点$\boldsymbol{\xi_i}$有
$$
